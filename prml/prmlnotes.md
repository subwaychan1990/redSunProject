...
1.  [Introduction](#org4d8a1ce)
        1.  [main](#orgf715dea)
    1.  [Example: Polynomial Curve Fitting](#orgbe98045)
    2.  [packages](#orgf0ab631)
2.  [Appendix A](#orgf2142f6)
...
Consider the example of recognizing handwritten digits, illustrated in Figure [2](#org1878867). Each digit corresponds to a \(28×28\) pixel image and so can be represented by a vector x comprising 784 real numbers.
...
The data for this example is generated from the function \(sin(2πx)\) with random noise included in the target values, as described in detail in [Appendix A](#org9fbd70b).
...
The geometrical interpretation(解释) of the sum-of-squares error function is illustrated in Figure [20](#org773f171).

![img](img/fig:1.3.png "The error function \eqref{error function} corresponds to (Mone half of) the sum of the squares of the displacements (shown by the vertical green bars) of each data point from the function \(y(x, \mathbf{w})\).")
...
In Figure [23](#org888d1f5), we show four examples of the results of fitting polynomials having orders \(M = 0, 1, 3, 9\) to the data set shown in Figure [15](#orgfce6d07).
...
![img](img/fig:1.4.png "Plots of polynomials having various orders M, shown as red curves, fitted to the data set shown in Figure 1.2.")
...
The third order \((M = 3)\) polynomial seems to give the best fit to the function \(sin(2πx)\) of the examples shown in Figure [23](#org888d1f5).
...
Graphs of the training and test set RMS errors are shown, for various values of M, in Figure [27](#org5b837e0).
The test set error is a measure of how well we are doing in predicting the values of \(t\) for new data observations of \(x\).
We note from Figure [27](#org5b837e0) that small values of \(M\) give relatively large values of the test set error, and this can be attributed(归结于) to the fact that the corresponding **polynomials are rather inflexible and are incapable of capturing the oscillations(震荡) in the function** \(sin(2πx)\).
Values of \(M\) in the range \(3 \leqslant M \leqslant 8\) give small values for the test set error, and these also give reasonable representations of the generating function \(sin(2πx)\), as can be seen, for the case of \(M = 3\), from Figure [23](#org888d1f5).
...
![img](img/fig:1.5.png "Graphs of the root-mean-square error, defined by \eqref{RMS}, evaluated on the training set and on an independent test set for various values of \(M\).")
...
<table id="org7da6aaa" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 1:</span> Table of the coefficients \(\mathbf{w^*}\) for polynomials of various order. Observe how the typical magnitude of the coefficients increases dramatically as the order of the polynomial increases.</caption>
...
<th scope="col" class="org-right">\(M=0\)</th>
<th scope="col" class="org-right">\(M=1\)</th>
<th scope="col" class="org-right">\(M=3\)</th>
<th scope="col" class="org-right">\(M=9\)</th>
</tr>
</thead>

<tbody>
<tr>
<td class="org-left">\(w_0^*\)</td>
...
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">-31.26</td>
<td class="org-right">-1553.7</td>
</tr>
...
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">20.41</td>
<td class="org-right">14489.7</td>
</tr>
...
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">342874</td>
</tr>


<tr>
<td class="org-left">\(w_8^*\)</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">-185879</td>
</tr>


<tr>
<td class="org-left">\(w_9^*\)</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">&#xa0;</td>
<td class="org-right">42105.2</td>
</tr>
</tbody>
</table>
...
However, the test set error has become very large and, as we saw in Figure [23](#org888d1f5), the corresponding function $y(x, \mathbf{w^⋆}) exhibits wild oscillations.
...
We can gain some insight into the problem by examining the values of the coefficients \(\mathbf{w^*}\) obtained from polynomials of various order, as shown in Table [1](#org7da6aaa).
We see that, as \(M\) increases, the magnitude of the coefficients typically gets larger.
In particular for the \(M = 9\) polynomial, the coefficients have become finely tuned to the data by developing large positive and negative values so that the corresponding polynomial function matches each of the data points exactly, but between data points (particularly near the ends of the range) the function exhibits the large oscillations observed in Figure [23](#org888d1f5).
...
It is also interesting to examine the behavior of a given model as the size of the data set is varied, as shown in Figure [31](#org5e5a0b0).
...
![img](img/fig:1.6.png "Plots of the solutions obtained by minimizing the sum-of-squares error function using the \(M = 9\) polynomial for \(N = 15\) data points (left plot) and \(N = 100\) data points (right plot). We see that increasing the size of the data set reduces the over-fitting problem.")
...
If, however, we use too large a value for λ then we again obtain a poor fit, as shown in Figure [35](#orgd4bdc47) for \(\ln\lambda = 0\).
The corresponding coefficients from the fitted polynomials are given in Table [2](#orgb47f425), showing that regularization has the desired effect of reducing the magnitude of the coefficients.
...
<table id="orgb47f425" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 2:</span> Table of the coefficients \(\mathbf{w^*}\) for \(M = 9\) polynomials with various values for the regularization parameter &lambda;. Note that \(\ln\lambda = -\infty\) corresponds to a model with no regularization, i.e., to the graph at the bottom right in Figure 1.4. We see that, as the value of &lambda; increases, the typical magnitude of the coefficients gets smaller.</caption>
...
<tr>
<th scope="col" class="org-left">&#xa0;</th>
<th scope="col" class="org-right">\(\ln\lambda=-18\)</th>
<th scope="col" class="org-right">\(\ln\lambda=-\infty\)</th>
<th scope="col" class="org-right">\(\ln\lambda=0\)</th>
</tr>
</thead>
...
<tr>
<td class="org-left">\(w_1^*\)</td>
<td class="org-right">9.48</td>
<td class="org-right">72.28</td>
...
<tr>
<td class="org-left">\(w_2^*\)</td>
<td class="org-right">-25.92</td>
<td class="org-right">-1554.91</td>
<td class="org-right">-0.42</td>
</tr>
...
<tr>
<td class="org-left">\(w_3^*\)</td>
<td class="org-right">2.61</td>
<td class="org-right">14500.6</td>
...
<tr>
<td class="org-left">\(w_4^*\)</td>
<td class="org-right">88.07</td>
<td class="org-right">-71265.4</td>
...
<tr>
<td class="org-left">\(w_5^*\)</td>
<td class="org-right">-144.36</td>
<td class="org-right">202305</td>
<td class="org-right">-0.07</td>
</tr>
...
<tr>
<td class="org-left">\(w_7^*\)</td>
<td class="org-right">137.65</td>
<td class="org-right">343094</td>
...
<tr>
<td class="org-left">\(w_8^*\)</td>
<td class="org-right">88.87</td>
<td class="org-right">-185995</td>
...
<tr>
<td class="org-left">\(w_9^*\)</td>
<td class="org-right">-117.89</td>
<td class="org-right">42130.9</td>
<td class="org-right">0.15</td>
</tr>
</tbody>
</table>

The impact of the regularization term on the generalization error can be seen by plotting the value of the RMS error \eqref{RMS} for both training and test sets against \(\ln\lambda\), as shown in Figure [38](#org74ff6b6).
...
![img](img/fig:1.8.png "Graph of the root-mean-square error \eqref{RMS} versus \(\ln\lambda\) for the \(M = 9\) polynomial.")
